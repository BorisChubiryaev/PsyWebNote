  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const regResult = await register(profile.email, `google_${profile.sub}`, profile.name || profile.email.split('@')[0]);
        if (regResult.success) { navigate('/onboarding'); }
        else setError(regResult.error || 'Ошибка входа через Google');
      } catch (e: unknown) {
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/60 p-6 border border-gray-100">
          ...
          <div className="space-y-3 mb-6">
            ...
            {/* VK */}
            <VKLoginButton disabled={loading || googleLoading} />
          </div>
