...
import {
  ...
  Clock, X, Calendar as CalendarIcon, List, Grid3X3, Edit, AlertCircle
} from 'lucide-react';
...
export default function Calendar() {
  const { appointments, clients, addAppointment, addSession, user, ensureSessionForAppointment } = useApp();
  ...
  // ── Navigate to session ─────────────────────────────────────────────────────
  const handleAptClick = (apt: typeof appointments[0]) => {
    const sessionId = ensureSessionForAppointment(apt);
    navigate(`/clients/${apt.clientId}/sessions/${sessionId}`);
  };

  const handleEditApt = (e: React.MouseEvent, apt: typeof appointments[0]) => {
    e.stopPropagation();
    const sessionId = ensureSessionForAppointment(apt);
  ...
  // ── Appointment card ────────────────────────────────────────────────────────
  const renderAptCard = (apt: typeof appointments[0], compact = false) => {
    const statusColors = {
    ...
    return (
      <div
        ...
        className={`rounded-lg cursor-pointer transition-all hover:scale-[1.02] active:scale-95 ${compact ? 'p-1.5' : 'p-3'} ${statusColors[apt.status as keyof typeof statusColors] || 'bg-gray-100'}`}
      >
        <div className={`font-medium truncate ${compact ? 'text-xs' : 'text-sm'}`}>{apt.clientName}</div>
        ...
        {!compact && (
          <div className="flex items-center gap-2 mt-2">
            ...
            <button onClick={e => handleEditApt(e, apt)}
    ...
    <Layout>
      <div className="max-w-7xl mx-auto animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h1 className="text-xl lg:text-3xl font-bold text-gray-900">Календарь</h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              {([
                ...
                <button key={v.mode} onClick={() => setViewMode(v.mode)}
                  className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all ${
                    viewMode === v.mode ? 'bg-white shadow text-indigo-600' : 'text-gray-600 hover:text-gray-900'
                  }`}>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
              ...
              <div className="flex items-center gap-1">
                ...
                <h2 className="text-sm sm:text-base font-semibold text-gray-900 min-w-[140px] sm:min-w-[200px] text-center capitalize">
                  {getHeaderTitle()}
                </h2>
        ...
        {viewMode === 'week' && (
          <div className="card overflow-x-auto p-0">
            <div className="min-w-[640px] p-4">
              {/* Header row */}
              <div className="grid grid-cols-8 gap-1 pb-3 border-b mb-2">
                ...
                {weekDays.map(day => (
                  <div key={day.toString()} className={`text-center ${isToday(day) ? 'text-indigo-600' : ''}`}>
                    <div className="text-xs text-gray-500">{format(day, 'EEE', { locale: ru })}</div>
                    <button
                      onClick={() => { setCurrentDate(day); setViewMode('day'); }}
                      className={`text-lg font-bold w-9 h-9 rounded-full mx-auto flex items-center justify-center ${
                        isToday(day) ? 'bg-indigo-500 text-white' : 'hover:bg-gray-100'
                      }`}
                    >
                      {format(day, 'd')}
                    </button>
                    ...
                    {weekDays.map(day => {
                      ...
                      return (
                        <div key={day.toString()}
                          ...
                          className="border border-gray-100 rounded-lg p-1 hover:bg-gray-50 cursor-pointer min-h-[52px] space-y-0.5"
                        >
                  ...
                  return (
                    <div key={hour.toString()} className="flex gap-3 min-h-[72px]">
                      ...
                      <div onClick={() => openAddModal(currentDate)}
                        className="flex-1 border border-gray-100 rounded-xl p-2 hover:bg-gray-50 cursor-pointer transition-colors">
                        {hourApts.length > 0 ? (
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-4">
                    ...
                    {dayApts.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 mb-2">Клиенты сегодня</h4>
                        <div className="space-y-2">
                          {dayApts.map(apt => (
                            <button key={apt.id}
                              onClick={() => handleAptClick(apt)}
                              className="w-full flex items-center gap-3 p-2 rounded-xl hover:bg-gray-100 transition-colors text-left">
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-black/60 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 animate-fadeIn">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md p-5 animate-slideUp max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-gray-900">Новая сессия</h3>
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl hover:bg-gray-50 border border-gray-100">
                  ...
                  <div>
                    <span className="text-sm font-medium text-gray-700">Онлайн-сессия</span>
                ...
                {newApt.isOnline && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1.5">Ссылка на встречу</label>
