...
export default function Calendar() {
  const { appointments, clients, addAppointment, ensureSessionForAppointment } =
    useApp();
  ...
  const selectedDateAppointments = useMemo(() => {
    ...
    return (appointmentsByDate[format(selectedDate, "yyyy-MM-dd")] || []).sort(
  ...
  const handleAddAppointment = async () => {
    ...
    await addAppointment({
      ...
      meetingLink: newApt.isOnline
        ? newApt.meetingLink || client.meetingLink
        : undefined,
    });
  ...
  const getAppointmentsForHour = (day: Date, hour: number) => {
    ...
    return (appointmentsByDate[dateKey] || []).filter((a) =>
  ...
  const handleAptClick = useCallback(
    async (apt: Appointment) => {
      ...
      try {
        ...
        if (sessionId) {
          ...
          console.error(
            "Could not find or create session for appointment",
            apt.id,
          );
        }
      } finally {
  ...
  const handleEditApt = useCallback(
    async (e: React.MouseEvent, apt: Appointment) => {
      e.stopPropagation();
  ...
  const renderAptCard = (apt: Appointment, compact = false) => {
    const isNavigating =
      navigatingId === apt.id || navigatingId === apt.id + "_edit";
    const statusColors = {
    ...
    return (
      ...
      >
        <div
          className={`font-medium truncate flex items-center gap-1 ${compact ? "text-xs" : "text-sm"}`}
        >
          {isNavigating && (
        ...
        <div
          className={`flex items-center gap-1.5 mt-0.5 ${compact ? "text-[10px]" : "text-xs"} opacity-80`}
        >
                      ...
                      {dayApts.length > 0 && (
                        <div className="mt-0.5 space-y-0.5">
                          {dayApts.slice(0, 2).map((apt) => (
                            <div
                              key={apt.id}
                              className={`text-[9px] sm:text-xs px-1 py-0.5 rounded truncate ${
                          ...
                          {dayApts.length > 2 && (
                            <div className="text-[9px] sm:text-xs text-gray-500 px-1">
                              +{dayApts.length - 2}
                            </div>
                          )}
                        </div>
            ...
            <div className="card h-fit lg:sticky lg:top-8">
              ...
              {selectedDateAppointments.length > 0 ? (
                <div className="space-y-3">
                  {selectedDateAppointments.map((apt) => renderAptCard(apt))}
                </div>
              ) : (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <button
                    onClick={() => selectedDate && openAddModal(selectedDate)}
            ...
            <div className="min-w-[640px] p-4">
              <div className="grid grid-cols-8 gap-1 pb-3 border-b dark:border-gray-700 mb-2">
                ...
                {weekDays.map((day) => (
                  <div
                    key={day.toString()}
                    className={`text-center ${isToday(day) ? "text-indigo-600" : ""}`}
                  >
              ...
              <div className="space-y-0.5">
                {dayHours.map((hour) => (
                  <div
                    key={hour.toString()}
                    className="grid grid-cols-8 gap-1 min-h-[52px]"
                  >
                    <div className="text-xs text-gray-400 pt-1 text-right pr-2">
                      {format(hour, "HH:00")}
                    </div>
                    {weekDays.map((day) => {
                      const hourApts = getAppointmentsForHour(
                        day,
                        hour.getHours(),
                      );
                      return (
          ...
          <div className="grid lg:grid-cols-3 gap-5">
              ...
              <div className="space-y-1">
                {dayHours.map((hour) => {
                  ...
                  return (
                    <div
                      key={hour.toString()}
                      className="flex gap-3 min-h-[72px]"
                    >
                      <div className="w-12 text-xs text-gray-400 pt-3 text-right flex-shrink-0">
                        {format(hour, "HH:00")}
                      </div>
                      <div
            ...
            <div className="card h-fit lg:sticky lg:top-8">
              ...
              {(() => {
                ...
                const dayApts = appointmentsByDate[dateKey] || [];
                const scheduled = dayApts.filter(
                  (a) => a.status === "scheduled",
                ).length;
                const completed = dayApts.filter(
            ...
            <div className="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md p-5 animate-slideUp max-h-[90vh] overflow-y-auto">
              ...
              {hasConflict && (
                <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-xl flex items-start gap-2 text-red-700 text-sm">
                  <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />В это
                  время уже запланирована другая сессия.
                </div>
              )}
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-100 dark:border-gray-600">
                  <input
                    type="checkbox"
                    checked={newApt.isOnline}
                    onChange={(e) =>
                      setNewApt({ ...newApt, isOnline: e.target.checked })
                    }
                ...
                {newApt.isOnline && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewApt({ ...newApt, meetingLink: e.target.value })
                      }
